...
**Every sheet in `assets/` is quantized to the master palette** (`docs/palette.json`, 56 colours in 7 ramps; reasoning and maintenance rules in `docs/palette.md`). `python tools/quantize.py <src> assets/<name>.png` is how a sheet enters `assets/`, and `python tools/quantize.py --check assets/` must report 100 % — a stray colour is a defect. Sources stay outside `assets/` so a palette change re-quantizes from the original. `tools/palette_sample.py` re-derives a proposal from the packs; it proposes, a human approves.
