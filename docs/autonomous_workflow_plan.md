...
**As built.** Two skills under `.claude/skills/`, each a numbered procedure an agent with no memory of this session can follow: `headless-test` (guard the generator, build with MinGW on `PATH`, pick or write a scenario with its predictions written as `#` comments beside each checkpoint, run, assert exact telemetry values, look at both PNGs, report per checkpoint) and `generate-asset` (search first, look at the sheet, stop if it fits; on a miss check `get_balance` and refuse below 20 remaining; generate at 16 px per cell with the palette strip forced; quantize; `--check` at 100 %; reconfigure). `tools/gen_asset.py` is dropped as the open items said — the skill calls the PixelLab MCP tools directly. `tools/index_packs.py` builds `artifacts/pack_manifest.json` (481 sheets in ~4 s; a gitignored cache, since it describes a folder outside the repo) and answers `--search "<words>"`: size, inferred cell with how it was inferred (`seams` measured, `single`, or `assumed` 16 for a gapless atlas), occupied cells, colour count, and how much of the sheet already sits on the master palette. Filenames alone cannot find a locker inside `Furniture.png`, so a hand-written `SHEET_CONTENTS` table names what the multi-object sheets hold, plus a small synonym map (`pencil` → `pen`, `quill`).

**Verified the plan's way.** `/headless-test` on `pickup`: all four predictions hold (bag `MUSHROOM ×1`, unchanged on the miss, `×2` after the step; `frame.png` shows the hotbar count). Then with the 3×3 pickup scan cut to 1×1 the same run reports an empty bag at `after_pickup` — a FAIL against the prediction — and the edit was reverted. `/generate-asset` search path: "locker" finds `BCKRMlv1_Prop_set.png` and `PostApoc_Workshop.png`; "pencil" finds the quill in Pixel Crawler's `Esoteric.png`, which closes the Phase 5 note that a pencil would need a generation — it does not. No generations were spent; the trial stands at 36 remaining.
