...
| Map mechanics | Craftable magic book of maps, minimap rendering |
...
Failures print the seed so any break is reproducible.

---

## Building

**Requirements:** CMake 3.14+, a C++20 compiler (MSVC 2019+, GCC 10+, Clang 12+). Raylib 5.0, Dear ImGui, rlImGui and GoogleTest are fetched automatically via CMake `FetchContent` — no manual dependency setup.
...
Run `Backrooms` for the game and `BackroomsTests` for the test suite. Assets are copied into the build directory automatically.
